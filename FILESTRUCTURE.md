...
* C++ source files:
  * `include/raspi_stock_reporter/raspi_stock_reporter.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/raspi_stock_reporter.cpp` is the main file that implements this library.
  * `app/raspi_stock_reporter_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/raspi_stock_reporter_t.cpp` contains the unit tests for the library.
...
* CMake build system files
  ...
  * `raspi_stock_reporterConfig.cmake.in` provides a template for the configuration
    installed alongside your project. This is required to implement the transitivity of your dependency
    on `libcurl`: If downstream projects use your library, they should
    automatically search for `libcurl`. The config file template implements
    exactly this logic.
